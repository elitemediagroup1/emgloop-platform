...
function tgAdapter(fetch: () => Promise<readonly TelegramMessageFacts[]>): TelegramAdapter {
  const port: TelegramClientPort = { async connectFromSession() { return { kind: 'telegram-mtproto', client: {} }; }, fetchSince: fetch, async fetchHistory() { return []; }, async fetchContentSince() { return []; }, async close() {} };
...
test('a stale session records RECONNECT_REQUIRED and sinks nothing', async () => {
  ...
  const authFailPort: TelegramClientPort = { async connectFromSession() { throw Object.assign(new Error('x'), { name: 'AuthError' }); }, async fetchSince() { return []; }, async fetchHistory() { return []; }, async fetchContentSince() { return []; }, async close() {} };
