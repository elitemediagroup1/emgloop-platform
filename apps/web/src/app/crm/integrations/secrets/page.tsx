...
import { allSecretRefs, listProviders } from '@emgloop/database';
import { IntegrationOsService } from '@emgloop/database';

// Secret Status â Sprint 16 (admin-only, status only).
//
// Consolidated view of every server environment variable referenced by the
// integration catalog. Reports CONFIGURED / MISSING (boolean) only â values
    ...
    <>
      <div className="crm-wf-head">
        <div>
          ...
          <p className="crm-sub">
            {configuredCount} of {rows.length} configured. Status only â no secret
        ...
        {rows.map((r) => (
          <div key={r.envVar} className="ios-secret">
            <div>
              ...
              <div className="masked">{r.configured ? 'â¢â¢â¢â¢â¢â¢â¢â¢â¢â¢â¢â¢â¢â¢â¢â¢' : 'not set'}</div>
              <div className="crm-sub" style={{ fontSize: '0.7rem', marginTop: '0.2rem' }}>Used by: {r.usedBy.join(', ')}{r.required ? ' Â· required' : ' Â· optional'}</div>
            </div>
