            ...
            <div className="loop-card">
              ...
              {decisions.length > 0 ? (
                <div className="loop-attn">
                  {decisions.map((d, i) => (
                    <AttentionRow key={i} icon={d.icon} tone={d.tone} title={d.title} detail={d.detail} href="/app/admin/marketplace/vendors" />
                  ))}
                </div>
              ) : (
                <div className="loop-empty loop-empty--good">
                  <p className="loop-empty__title">Nothing needs review</p>
                  <p className="loop-empty__body">No vendor issues are surfaced by the current data.</p>
                </div>
              )}
            </div>
