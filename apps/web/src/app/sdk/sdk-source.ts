// sdk-source.ts - Sprint 17 (Connect the Real World).
//
// The REAL EMG Loop browser SDK source, served verbatim at /sdk/emg-loop.js.
// It is authored as an array of source lines (joined at module load) so the
// file stays pure ASCII and needs no build step - the route handler returns
// EMG_LOOP_SDK_SOURCE with a JavaScript content type. No third-party
// dependencies; the SDK is plain ES5-compatible browser code.
//
// The SDK batches first-party website events and POSTs them to the Website
// webhook (/api/webhooks/website), which verifies + ingests them through the
// same pipeline as every other provider. Per-property configuration is read
// from the loading <script> tag's data-* attributes.

const SDK_LINES: string[] = [
  ...
  "",
  "  // ---- Resolve configuration from the loading <script> tag ----------------",
  "  var current =",
  ...
  "  };",
  "",
  "  // ---- Storage helpers (safe across privacy modes) ------------------------",
  ...
  "  }",
  "",
  "  // ---- Identity: visitor id (persistent) + session id (30 min sliding) -----",
  "  function uuid() {",
  ...
  "",
  "  // Identity stitching: when the site identifies a visitor (login, form), call",
  "  // window.emgLoop.identify({ email, phone, id }) and it is attached to events.",
  "  var identity = {};",
  ...
  "",
  "    // Click delegation: phone, email, CTA, outbound links, downloads.",
  ...
  "    }, true);",
  "",
  "    // Forms: start (first focus) + submit. Search + ZIP detection by field type.",
  "    var started = {};",
  ...
  "      if (/search/i.test(name) || searchValue(f)) { track('search_performed', { query: q, form: name }); return; }",
  ...
  "      track('identify', {});",
  "    },",
  "    // Convenience wrappers for interactive features sites can call directly.",
...
export const EMG_LOOP_SDK_SOURCE: string = SDK_LINES.join('\n');

export const EMG_LOOP_SDK_VERSION = '1.0.0';
