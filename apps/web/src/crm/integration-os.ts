...
// Integration OS bridge â Sprint 16 (The Connection Layer).
//
// Server-only glue between the static provider catalog (@emgloop/brain) and the
// live status engine (@emgloop/database). The Integration Center pages call
// these helpers and render the result â no provider-specific code lives in the
...
export function computeSystemHealth(cards: ProviderCard[]): SystemHealth {
  ...
  for (const { spec, status } of cards) {
    ...
    if (status.missingRequiredSecrets.length > 0) {
      ...
      warnings += 1;
      missingItems.push(spec.displayName + ' not verified â awaiting first event');
    }
