...
| Budget (only if a notification address is given) | a monthly cost ceiling. Staging leaves it unset: its budget is the central $100 one (runbook step 5) |
...
All secrets are encrypted with the data key.
- **Kept once created:** they survive updates and stack deletion.
- **A rolled-back first creation deletes them** (`RetainExceptOnCreate`, as for the keys and log
  groups), so a failed first deployment leaves no fixed name behind.
- **No secret value appears in the template** (tested).
...
| delete `LoopBrain-staging` (runbook, "Remove the environment") | stateless resources removed; keys, secrets and log groups **kept** (`RetainExceptOnCreate`; termination protection must be turned off first) | messages are lost; jobs stay in Neon and the sweeper recovers them after a redeploy |
...
| Item | Value |
|---|---|
| Account | **Loop Brain Staging, `065148797865`** (Matt, 2026-09-17), a member account of the organization whose management account is EMG Loop Production, `670682108352`; never the management account. Pinned in `infra/brain/lib/target.ts`: the app refuses credentials for any other account, and the deploy workflow checks the role ARN and the credentials again |
| Region | `us-east-1` |
...
| Bootstrap | **required, not done.** In CloudShell, from a directory without `cdk.json`: `npx --yes aws-cdk@2.1142.0 bootstrap aws://065148797865/us-east-1 --termination-protection`. It creates the `CDKToolkit` stack (runbook step 14 lists its resources). The stack needs bootstrap version 6 or later and has no image assets. Run from `infra/brain`, the CLI would synthesize the app first and fail without its bundles |
| Deploy identity | **not created.** `infra/brain/access/github-deploy-access.yaml`, deployed once by hand as `LoopBrain-staging-github-access` (runbook step 9). It holds the GitHub OIDC provider and the role `loop-brain-github-deploy`.<br>• **Trust:** only `repo:elitemediagroup1/emgloop-platform:environment:brain-staging`, audience `sts.amazonaws.com`.<br>• **Permission:** only `sts:AssumeRole` on the CDK deploy, file-publishing and lookup roles (`test/access.test.ts`).<br>• **In effect:** the default CDK deploy role can change any stack here through the administrator execution role, so running the pipeline amounts to administering this account. The environment gates, the guardrails and the trail are the controls |
| Lambda quota | **10 today, needs 118** (18 reserved plus the 100 Lambda keeps unreserved). An increase to 1,000 is pending. Deployment waits for the applied value (runbook step 13); the bootstrap does not |
| Audit and budget | **not created.** An organization CloudTrail trail from `670682108352` (runbook step 4, before the bootstrap), and one central $100 monthly budget for the linked account (step 5) |
| Retention | keys, secrets and log groups use `RetainExceptOnCreate`: kept on update and deletion, deleted if their own first creation rolls back. A failed first deployment can simply be retried (stack test) |
...
| Deploy mechanism | `.github/workflows/brain-infra-deploy.yml`: manual, `diff` or `deploy`, typed confirmation, `brain-staging` environment (Matt as reviewer, `main` only; runbook step 11), OIDC through `aws-actions/configure-aws-credentials` pinned to the v6.3.0 commit with `allowed-account-ids: 065148797865`, and account checks before anything is compared. **Not run** |

**Deployment is stopped here.** It waits for:
- the organization trail and the budget (runbook steps 4–5);
- the deploy identity and the `brain-staging` environment (steps 9–11);
- an applied Lambda limit of at least 118 (step 13);
- the bootstrap (step 14), which needs Matt's go-ahead;
- staging Neon (Part 5), so its secrets can be written right after the deployment;
...
**The pre-deployment pass (B7, 2026-09-17; nothing created):**
- **The GitHub deploy identity is a committed template,** `infra/brain/access/github-deploy-access.yaml`,
  deployed once by hand. It stays outside the Brain app, so the pipeline never manages the identity it
  runs as, and it needs no bootstrap.
  - Tests pin its trust and its three role ARNs, and forbid wildcards.
  - They also check the ARNs against the roles the cloud assembly will ask CDK to assume.
- **A failed first deployment would have blocked its own retry.** Plain `Retain` keeps resources even
  when their creation rolls back, which would have left 6 fixed-name secrets and 5 fixed-name log
  groups (and 2 keys). With the account's Lambda limit at 10, a first deployment would fail. The 13
  now use `RetainExceptOnCreate`.
- **The runbook's bootstrap step would have failed.** It ran `cdk bootstrap` from `infra/brain`, where
  the CLI synthesizes the app first, and that fails without bundles. It now runs from CloudShell
  outside the repository, with the CLI pinned.
- **The credentials action moved from v4 to the v6.3.0 commit** (`e1253824…`), with
  `allowed-account-ids`. With OIDC, v6.3.0 sends no session tags, so the trust needs no
  `sts:TagSession`.
- **The runbook now covers the governance steps:** the management account (`670682108352`), the
  organization trail, the central $100 budget (the stack's own budget stays off), the exact
  `brain-staging` environment settings, and the applied-quota check.
