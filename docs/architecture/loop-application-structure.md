...
- Home: Home, Mail, Chats, Calendar, Connections, all primary (Chats and Calendar since 2026-09-24; Connections is configuration only).
...
| 2026-09-24 | **Chats and Calendar are domain pages** (amends D5): Chats and Calendar are domain pages (read-only compositions of the viewer's own Telegram obligations/activity and calendar); Connections is configuration only. `/app/chats` (gate `googleWorkspace:view`, the Connections page's) projects the viewer's own triage items, connection state and content-free activity through one pure projection (`chatsIntelligence`) that Home's Chats tile reads too; `/app/calendar` (gate `employeeIntelligence:view`, Mail's) draws `loadYourDay`. Both sit in the Home group after Mail and before Connections, and link to Connections only to connect or manage a source. No table, migration, prompt, permission or client. |
