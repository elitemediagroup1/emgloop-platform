...
test('the shipped routing policy conforms for every task, and Case Explanation needed no change', () => {
  ...
  // The version moves when a ROUTE is added, and only then. GM-3 added Mail Reply Draft; Case
  // Explanation's own entry is untouched, which is what the assertions around this one check.
  assert.equal(AI_ROUTING_POLICY_VERSION, 'routing.2026-09-18.3');
