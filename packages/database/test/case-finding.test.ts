...
test('a measurement-backed finding establishes when the Stage 3 verdict is READY', async () => {
  ...
  const finding = await findings.get(ORG, caseId, NOW);
  assert.equal(finding?.evidenceState, 'ESTABLISHED');
  assert.equal(finding?.establishment.basis, 'DETERMINISTIC_POLICY');
  // NO ACTOR, BECAUSE NO PERSON DECIDED. A policy establishment is an evaluation.
  assert.equal(finding?.judgment, null, 'nobody judged it, and none was needed');
...
test('establishment is DERIVED — the same stored finding un-establishes when its evidence degrades', async () => {
  ...
  const { findingId } = await findings.record(ORG, caseId, CLAIM);
  assert.equal((await findings.get(ORG, caseId, NOW))?.evidenceState, 'ESTABLISHED');
  ...
  assert.equal(after?.evidenceState, 'DEVELOPING');
...
test('a person accepting a non-measurement finding does NOT establish it', async () => {
  // THE CORE OF STAGE 5 PR 1. Human authority may authorize action under
  // uncertainty; it cannot authorize certainty. Before this, acceptance made the
  // claim ESTABLISHED outright.
  const { findings, caseId } = await world();
  ...
  await findings.accept(ORG, caseId, findingId!, HUMAN.userId);

  const finding = await findings.get(ORG, caseId, NOW);
  assert.equal(finding?.evidenceState, 'DEVELOPING');
  assert.equal(finding?.establishment.basis, null);
  // AND IT SAYS WHY, rather than leaving a person to guess why their acceptance
  // did not move it.
  assert.deepEqual(finding?.establishment.reasons, ['CLAIM_NOT_MEASUREMENT_BACKED']);
  // The judgement is recorded in full, on its own axis.
  assert.equal(finding?.judgment?.judgment, 'ACCEPTED');
  assert.equal(finding?.judgment?.byUserId, HUMAN.userId);
  assert.ok(finding?.judgment?.at);
  assert.equal(finding?.lifecycle, 'CURRENT');
});

test('accepting a developing measurement-backed claim does not establish it either', async () => {
  // No verdict can be obtained, so the gate fails closed — and stays closed
  // however senior the person pressing accept is.
  const { findings, caseId } = await world({ readiness: null });
  const { findingId } = await findings.record(ORG, caseId, CLAIM);
  await findings.accept(ORG, caseId, findingId!, HUMAN.userId);
...
test('acceptance still refuses an unattributed actor', async () => {
  const { findings, caseId } = await world();
  const { findingId } = await findings.record(ORG, caseId, CLAIM);
  await assert.rejects(() => findings.accept(ORG, caseId, findingId!, '  '), /attributed actor/);
});

test('a rejected finding keeps being evaluated, and can be established', async () => {
  // REJECTION IS A JUDGEMENT, NOT EVIDENCE. Before this, rejecting a claim made
  // it "not live" and the gate refused to look at it at all, so a person's
  // disagreement silently removed its evidentiary standing.
  const { findings, caseId, hypotheses } = await world();
  const { findingId } = await findings.record(ORG, caseId, CLAIM);
  await findings.reject(ORG, caseId, findingId!, HUMAN.userId);
  ...
  const finding = await findings.get(ORG, caseId, NOW);
  // Established AND rejected: two true facts on two axes.
  assert.equal(finding?.evidenceState, 'ESTABLISHED');
  assert.equal(finding?.establishment.basis, 'DETERMINISTIC_POLICY');
  assert.deepEqual(finding?.establishment.reasons, []);
  assert.equal(finding?.judgment?.judgment, 'REJECTED');
  assert.equal(finding?.judgment?.byUserId, HUMAN.userId);
  assert.ok(finding?.judgment?.at);
  // It is still the claim on the Case. Rejection is not a lifecycle.
  assert.equal(finding?.lifecycle, 'CURRENT');
});

test('a rejection does not weaken the evidence — the same claim reads the same either way', async () => {
  const { findings, caseId } = await world();
  const { findingId } = await findings.record(ORG, caseId, CLAIM);
  ...
  assert.equal(before?.evidenceState, after?.evidenceState);
  assert.deepEqual(before?.establishment, after?.establishment);
  assert.equal(before?.judgment, null);
  assert.equal(after?.judgment?.judgment, 'REJECTED');
});

test('a judgement is scoped to the Case — another Case\'s claim fails closed', async () => {
  // SAME TENANT, DIFFERENT CASE. The id is real and the organization is right;
  // it is simply not this investigation's claim, and it gets the same null a
  // claim that does not exist would.
  const { findings, engine, caseId, hypotheses } = await world();
  const { findingId } = await findings.record(ORG, caseId, CLAIM);
  ...
  await findings.record(ORG, other.id, { ...CLAIM, claim: 'A different claim.' });

  assert.equal(await findings.accept(ORG, other.id, findingId!, HUMAN.userId), null);
  assert.equal(await findings.reject(ORG, other.id, findingId!, HUMAN.userId), null);
  assert.equal((await hypotheses.findById(ORG, findingId!))?.status, 'PROPOSED');
});

test('a superseded ancestor cannot be judged — only the current claim can', async () => {
  const { findings, caseId, hypotheses } = await world();
  ...
  assert.equal(await findings.accept(ORG, caseId, first.findingId!, HUMAN.userId), null);
  assert.equal(await findings.reject(ORG, caseId, first.findingId!, HUMAN.userId), null);
  assert.equal((await hypotheses.findById(ORG, first.findingId!))?.status, 'SUPERSEDED');
});

test('a judgement naming a claim or Case that does not exist writes nothing', async () => {
  const { findings, caseId } = await world();
  await findings.record(ORG, caseId, CLAIM);
  assert.equal(await findings.accept(ORG, caseId, 'fnd_nope', HUMAN.userId), null);
  assert.equal(await findings.reject(ORG, caseId, '', HUMAN.userId), null);
  assert.equal(await findings.accept(ORG, 'case_nope', 'fnd_nope', HUMAN.userId), null);
});
...
test('supersession preserves the previous finding in full', async () => {
  ...
  assert.equal(finding?.lineage[0]?.lifecycle, 'SUPERSEDED');
...
test('a finding cannot be accepted or rejected across a tenant boundary', async () => {
  const { findings, caseId, hypotheses } = await world();
  const { findingId } = await findings.record(ORG, caseId, CLAIM);
  assert.equal(await findings.accept(OTHER_ORG, caseId, findingId!, HUMAN.userId), null);
  assert.equal(await findings.reject(OTHER_ORG, caseId, findingId!, HUMAN.userId), null);
  assert.equal((await hypotheses.findById(ORG, findingId!))?.status, 'PROPOSED');
});
