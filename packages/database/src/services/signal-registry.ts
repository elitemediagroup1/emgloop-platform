...
export const SIGNAL_REGISTRY: Record<string, SignalDefinition> = {
  homeowner_candidate: { key: 'homeowner_candidate', type: 'CUSTOM', label: 'Homeowner Candidate', description: 'Inbound contact about a home service suggests a homeowner.' },
  emergency_intent: { key: 'emergency_intent', type: 'INTENT', label: 'Emergency Intent', description: 'Language or context implies an urgent / emergency need.' },
  phone_preference: { key: 'phone_preference', type: 'CUSTOM', label: 'Phone Preference', description: 'Customer engages via phone — prefer voice for outreach.' },
  service_interest: { key: 'service_interest', type: 'INTENT', label: 'Service Interest', description: 'Customer expressed interest in a specific service.' },
  location_known: { key: 'location_known', type: 'CUSTOM', label: 'Location', description: 'A service location / area was captured for the customer.' },
  time_of_day_behavior: { key: 'time_of_day_behavior', type: 'CUSTOM', label: 'Time-of-day Behavior', description: 'When during the day the customer tends to engage.' },
  journey_stage: { key: 'journey_stage', type: 'CUSTOM', label: 'Customer Journey Stage', description: 'Where the customer sits in the lifecycle (new, engaged, ...).' },
  // --- Sprint 14: website behaviour signals ---
  web_preference: { key: 'web_preference', type: 'CUSTOM', label: 'Web Preference', description: 'Customer engages via website — a second sense for the Brain.' },
  research_intent: { key: 'research_intent', type: 'INTENT', label: 'Research Intent', description: 'Browsing guides / content without yet converting.' },
  comparison_shopper: { key: 'comparison_shopper', type: 'INTENT', label: 'Comparison Shopper', description: 'Repeated searches / multiple categories suggest comparison.' },
  buying_intent: { key: 'buying_intent', type: 'INTENT', label: 'Buying Intent', description: 'CTA / phone / appointment actions indicate purchase intent.' },
  appointment_intent: { key: 'appointment_intent', type: 'INTENT', label: 'Appointment Intent', description: 'Requested an appointment via a website surface.' },
  download_intent: { key: 'download_intent', type: 'INTENT', label: 'Download Intent', description: 'Downloaded a resource — active evaluation behaviour.' },
  returning_visitor: { key: 'returning_visitor', type: 'CUSTOM', label: 'Returning Visitor', description: 'Visitor has multiple website sessions over time.' },
  highly_engaged: { key: 'highly_engaged', type: 'CUSTOM', label: 'Highly Engaged', description: 'Deep / repeated interaction across website surfaces.' },
  high_value_prospect: { key: 'high_value_prospect', type: 'LIFETIME_VALUE', label: 'High Value Prospect', description: 'Strong buying + appointment signals from website behaviour.' },
  newsletter_subscriber: { key: 'newsletter_subscriber', type: 'UPSELL_OPPORTUNITY', label: 'Newsletter Subscriber', description: 'Opted into the newsletter — a nurturing channel is open.' },
  commercial_buyer: { key: 'commercial_buyer', type: 'INTENT', label: 'Commercial Buyer', description: 'Commercial / business context detected in website activity.' },
  pet_owner: { key: 'pet_owner', type: 'TOPIC', label: 'Pet Owner', description: 'Engagement with PetsInMyCity / pet topics.' },
  caregiver: { key: 'caregiver', type: 'TOPIC', label: 'Caregiver', description: 'Engagement with CareInMyCity / caregiving topics.' },
  wedding_planning: { key: 'wedding_planning', type: 'TOPIC', label: 'Wedding Planning', description: 'Engagement with wedding planning topics.' },
  moving_soon: { key: 'moving_soon', type: 'INTENT', label: 'Moving Soon', description: 'Relocation / moving context detected in website activity.' },
  website_source: { key: 'website_source', type: 'CUSTOM', label: 'Website Source', description: 'Which EMG property the customer engaged with.' },
};

const EMERGENCY_WORDS = ['emergency', 'urgent', 'asap', 'right now', 'flood', 'leak', 'burst', 'no heat', 'no power', 'no ac', 'no air', 'gas', 'sparking', 'smoke'];
const SERVICE_WORDS = ['hvac', 'plumb', 'electric', 'roof', 'repair', 'install', 'furnace', 'heater', 'cooling', 'ac', 'air conditioning', 'drain', 'water heater'];
...
function text(event: NormalizedEvent): string {
  ...
  for (const k of ['campaign', 'source', 'transcript', 'keyword', 'service', 'note', 'query', 'page', 'title', 'category', 'property', 'cta']) {
...
export function deriveSignals(event: NormalizedEvent): DerivedSignal[] {
  ...
  const location = asString(meta['city']) ?? asString(meta['region']) ?? asString(meta['location']) ?? asString(meta['area']);
  if (location) {
...
function sig(
  ...
  return { key: def.key, type: def.type, label: def.label, ...extra };
}
