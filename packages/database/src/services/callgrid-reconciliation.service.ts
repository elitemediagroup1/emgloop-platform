...
const ENRICH_OTHER_KEYS = ['durationSeconds', 'revenue', 'payout', 'billable', 'paid'] as const;
...
export function mapReconEventType(raw: string): string {
  ...
  if (k.includes('answer')) return 'call.answered';
  if (k.includes('miss') || k.includes('no_answer') || k.includes('noanswer')) return 'call.missed';
  if (k.includes('voicemail')) return 'call.voicemail';
  if (k.includes('transfer')) return 'call.transferred';
  if (k.includes('complete') || k.includes('hangup')) return 'call.completed';
  return 'call.inbound';
}
